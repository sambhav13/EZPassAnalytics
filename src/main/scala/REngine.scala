...
class REngine extends Serializable {
...
object REngine extends Serializable{
...
import sqlContext.implicits._
	lines.foreachRDD(x =>{
	  ...
	  val df = x.map( o =>
	      ...
	      //df.show()
	      df.write.mode(SaveMode.Append).jdbc("jdbc:mysql://localhost:3306/DeadLine","message",prop);
	  })
	
	//DB Connection Setup
	  
	//val url = "jdbc:mysql://172.31.28.225:3306/ezcheckout3"
	...
	val racknCategory = sqlContext.read.format("jdbc").option("url", "jdbc:mysql://localhost:3306/DeadLine")
	.option("driver", "com.mysql.jdbc.Driver")
	...
	val Category = sqlContext.read.format("jdbc").option("url", "jdbc:mysql://localhost:3306/DeadLine")
	.option("driver", "com.mysql.jdbc.Driver")
	...
	lines.foreachRDD( (rdd: RDD[String], time: Time) => {
		...
		val locDf =   LocationEventData.map ( x =>  {
		...
		val LocationDF = locDf.filter(locDf("locEventType")===0)
...
//	finalAggregatedCategorySaleAmount.show()                                                    
  ...
  //////////////////////////////////////////////////////////////////////////
	 ...
	 val events_interim = sqlContext.read.format("jdbc").option("url", url)
	.option("driver", "com.mysql.jdbc.Driver")
	...
	val events = events_interim.filter(events_interim("locEventType")===0)
