...
class RetailAnalyticsEngine_Server extends Serializable {
...
object RetailAnalyticsEngine_Server extends Serializable{
...
import sqlContext.implicits._
	lines.foreachRDD(x =>{
	  ...
	  val df = x.map( o =>
	      ...
	      df.show()
	      df.write.mode(SaveMode.Append).jdbc("jdbc:mysql://172.31.28.225:3306/ezcheckout3","message",prop);
	  })
	/*
	//DB Connection Setup
